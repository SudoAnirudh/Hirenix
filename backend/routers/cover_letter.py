...
async def generate_cover_letter(
    ...
    # ⚡ Bolt: Offload synchronous db call to prevent blocking main event loop
...
async def export_cover_letter(
    ...
    # ⚡ Bolt: Fetch cover letter and user profile concurrently, unblocking the main thread
    r_task = asyncio.to_thread(lambda: (
    ...
    p_task = asyncio.to_thread(lambda: (
        db.table("profiles")
        .select("full_name")
        .eq("id", user["user_id"])
        .single()
