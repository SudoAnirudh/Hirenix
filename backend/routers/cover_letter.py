...
async def export_cover_letter(
    ...
    # What: Refactored individual Supabase `.execute()` queries into a concurrent `asyncio.gather` operation.
    # Why: The Supabase python client is synchronous; executing queries sequentially blocks the FastAPI async event loop.
    # Impact: Reduces latency of the `export_cover_letter` endpoint by executing both database queries in parallel.
