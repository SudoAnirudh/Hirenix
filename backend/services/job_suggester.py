...
async def get_user_readiness_context(user_id: str, db) -> Dict[str, Any]:
    ...
    try:
        ...
        # Impact: Reduces query time from ~4N to ~1N, significantly speeding up context generation.
        resumes_task = asyncio.to_thread(lambda: db.table("resumes").select("id, raw_text, ats_score").eq("user_id", user_id).order("created_at", desc=True).limit(1).execute())
