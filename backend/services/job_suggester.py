...
async def get_user_readiness_context(user_id: str, db) -> Dict[str, Any]:
    ...
    """
    try:
        # What: Run the 4 synchronous Supabase queries concurrently.
        ...
        # Fetch dependent data concurrently to prevent event loop blocking
        async def fetch_sections():
            if not resume_r.data: return None
            resume_id = resume_r.data[0]["id"]
            return await asyncio.to_thread(lambda: db.table("resume_sections").select("content").eq("resume_id", resume_id).eq("section_type", "skills").execute())

        async def fetch_answers():
            if not interviews_r.data: return None
            session_ids = [i["id"] for i in interviews_r.data]
            return await asyncio.to_thread(lambda: db.table("interview_answers").select("category, score").in_("session_id", session_ids).execute())

        sections_r, answers_r = await asyncio.gather(fetch_sections(), fetch_answers())
