...
async def get_user_readiness_context(user_id: str, db) -> Dict[str, Any]:
    ...
    """
    try:
        # ⚡ Bolt: Use asyncio.gather and to_thread for concurrent database calls
        # Why: Supabase python client is synchronous. Sequential calls block the event loop and increase latency.
        # Impact: Reduces N+1 database call latency by running these lookups concurrently.
        ...
        if interviews_r.data:
            ...
            # ⚡ Bolt: Execute follow-up query in a separate thread as well
