...
async def get_user_readiness_context(user_id: str, db) -> Dict[str, Any]:
    ...
    """
    try:
        # 1. Run independent queries concurrently
        resume_task = asyncio.to_thread(db.table("resumes").select("id, raw_text, ats_score").eq("user_id", user_id).order("created_at", desc=True).limit(1).execute)
        interviews_task = asyncio.to_thread(db.table("interview_sessions").select("id, overall_score, target_role").eq("user_id", user_id).order("created_at", desc=True).limit(3).execute)
        github_task = asyncio.to_thread(db.table("github_analyses").select("gpi_score, strengths").eq("user_id", user_id).order("created_at", desc=True).limit(1).execute)
        linkedin_task = asyncio.to_thread(db.table("linkedin_analyses").select("strengths").eq("user_id", user_id).order("created_at", desc=True).limit(1).execute)
        ...
        if resume_r.data:
            ...
            secondary_tasks.append(asyncio.to_thread(db.table("resume_sections").select("content").eq("resume_id", resume_id).eq("section_type", "skills").execute))
        else:
            secondary_tasks.append(asyncio.sleep(0))
        
        # 6. Ready Skills from Interviews (score >= 7.0)
        session_ids = []
        if interviews_r.data:
            session_ids = [i["id"] for i in interviews_r.data]
            secondary_tasks.append(asyncio.to_thread(db.table("interview_answers").select("category, score").in_("session_id", session_ids).execute))
        else:
            secondary_tasks.append(asyncio.sleep(0))

        secondary_results = await asyncio.gather(*secondary_tasks)
        sections_r = secondary_results[0] if secondary_results and resume_r.data else None
        answers_r = secondary_results[1] if len(secondary_results) > 1 and interviews_r.data else None
