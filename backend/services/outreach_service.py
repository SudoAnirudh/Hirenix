...
async def generate_outreach_drafts(
    ...
    """
    try:
        # ⚡ Bolt: Prevent blocking the async event loop with synchronous Supabase calls
        # What: Wrap Supabase `.execute()` calls in `asyncio.to_thread`.
        # Why: The Supabase python client is synchronous. Calling `.execute()` inside an `async def` blocks the entire event loop.
        # We process these sequentially instead of using `asyncio.gather` because later queries are conditional fallbacks, preserving early returns.
